...
class Detection_model(object):
    ...
    def detect(self, frame):
        ...
        '''
        results = self.model(frame, verbose=False, iou=0.5)[0]
...
class ParkingManager(object):
    ...
    def run(self):
        ...
        if self.save_table:
            parking_df = DataFrame(columns=['Car_ID', 'Parked_at', 'Left_at', 'Duration'])
        ...
        while self.cap.isOpened():
            ...
            # check if any car left
            leaving_IDs = [(id, track_history.pop(id)) for id in list(track_history) if id not in [track.track_id for track in tracks]]
            # print(leaving_IDs)
            if self.save_table:
                for id, parked_info in leaving_IDs:
                    parked_at = parked_info[0]['Parked_at']
            ...
            for track in tracks:
                ...
                if track_id not in track_history:
                    track_history[track_id].append({'Parked_at': datetime.now()})
                # plotting
...
if __name__ == "__main__":
    ...
    video_path = "./videos/test.mp4" #BLK-HDPTZ12 Security Camera Parkng Lot Surveillance Video.mp4
    ...
    print("Benchmark", (timeit.timeit(lambda: usage.run(), setup="pass",number=1))/60, "min")